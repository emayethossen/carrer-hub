...
const JobDetails = () => {
    ...
    useEffect(() => {
        for (const x of jobDetails) {
            const card = jobDetails.find(b => b.id == id)
            setData(card);
        }
    }, [])

    const { title, companyName, jobType, description, location, salary, responsibilities, phone, email, place, experience, education } = data
